...
public class AddressBookMain {
    ...
    private static void view_by_state_or_city_sorted() {
        ...
        switch (type) {
            case "state": {
                List<Contacts> sorted_by_state = contactList.stream()
    ...
    private static void search_by_city_or_state_get_multiple_person_count() {
        Map<String, Integer> city = contactList.parallelStream()
                .collect(Collectors.toConcurrentMap(Contacts::getCity, c -> 1, Integer::sum));
        Map<String, Integer> state = contactList.parallelStream()
                .collect(Collectors.toConcurrentMap(Contacts::getState, s -> 1, Integer::sum));
    ...
    private static void display_all_contact() {
        ...
        TreeMap<String, Contacts> sorted = new TreeMap<>();
    ...
    private static void add_contact_to_list(int index) {
        ...
        contactList.add(new Contacts(firstname, lastname, address, city, state, zip, phone_number, email));
        AddressBook.book.put(firstname, new Contacts(firstname, lastname, address, city, state, zip, phone_number, email));
    }
    ...
    private static void edit_contact_to_list(String name) {
        ...
        int index = (contactList.indexOf(AddressBook.book.get(name))) + 1;
        AddressBook.book.remove(name);
        contactList.set(index, new Contacts(firstname, lastname, address, city, state, zip, phone_number, email));
        AddressBook.book.put(firstname, new Contacts(firstname, lastname, address, city, state, zip, phone_number, email));
    }
}
